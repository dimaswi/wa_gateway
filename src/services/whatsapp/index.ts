  ...
  async initializeConnection(): Promise<WASocket | null> {
    ...
    sock.ev.on('messages.upsert', async m => {
      ...
      if (m.messages[0].key.participant) {
        ...
        const namaGroup = m.messages[0].key.remoteJid
        const text = m.messages[0]
        const pushName = m.messages[0].pushName
        const baseDir = `./wa-bots/chats/${namaGroup}/${pushName}`;

        const savedbText = await prisma.message.create({
          data: {
            title: pushName,
            pushName: pushName,
            remoteJid: namaGroup,
            groupName: namaGroup,
            pesan: JSON.stringify(text),
          }
        })
        // if (!await existsSync(baseDir)) {
        //   await mkdirSync(baseDir, { recursive: true });
        // }
        // await writeFileSync(`${baseDir}/${timeStamp}.json`, JSON.stringify(text))
      } else {
        const timeStamp = m.messages[0].messageTimestamp
        const namaGroup = m.messages[0].key.remoteJid
        const text = m.messages[0]
